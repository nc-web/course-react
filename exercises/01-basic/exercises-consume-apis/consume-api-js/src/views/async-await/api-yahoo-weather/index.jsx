...
const APIOpenWeather = () => {
  ...
  const dataLocation = async () => {
    try {
      const res = await fetch(`https://yahoo-weather5.p.rapidapi.com/weather?location=${stateCity}&format=json&u=f`, options)
      const data = await res.json()
      setStateLocation(data.location)
      dataForecasts()
    } catch (error) {
      window.alert(`Error city: ${error}`)
    }
  }
